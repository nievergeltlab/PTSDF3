...
tdata="ongb"
 ...
 dm1 <- fread(paste('data/p3_ONGB_87_V2.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))
 dm1$IID <- NULL #not matching actual IIDS
 
 dm1$Current_PTSD_Continuous <- dm1$pcl4_total
...
dm1 <- remove_empty(dm1, which = c("cols"))
 ...
 for (ancgroup in c("eur","aam"))
 {
  ...
  dm1x$Case <- dm1x$pcl4_dsm + 1
  
  dm1_exp <- subset(dm1x,!is.na(Current_PTSD_Continuous),select=c(FID,IID,Case))# Current_PTSD_Continuous,
 
  print(table(is.na(dm1_exp$Current_PTSD_Continuous)))
  
  
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_ongb","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_ongb","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
