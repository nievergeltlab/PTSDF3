...
tdata="auro"
 ...
 dm1 <- fread(paste('data/73_AURO_PCL5_pheno_dx_v2.csv',sep=''),data.table=F,na.strings=c(NA,"-9"))

 dm1$Current_PTSD_Continuous <- dm1$WK8_PCL5_RS
...
dm1 <- remove_empty(dm1, which = c("cols"))
 ...
 for (ancgroup in c("eur","aam"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_auro","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_auro","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
