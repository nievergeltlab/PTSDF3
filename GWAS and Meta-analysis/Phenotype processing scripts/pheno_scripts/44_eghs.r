...
#dm1 <- remove_empty(dm1, which = c("cols"))
 ...
 for (ancgroup in c("aam"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/p2_",tdata,"_eghs","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/p2_",tdata,"_",ancgroup,"_eghs","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
