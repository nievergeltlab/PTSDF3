...
tdata="nss2"
 ...
 for (ancgroup in c("eur","lat","aam"))
 {
  ...
  dm1x <- merge(dm1,dma,by=c("FID","IID"),suffixes=c("_fam","")) 
    ...
    dm1x$Case <- dm1x$Current_PTSD_Dx
  dm1_exp <- subset(dm1x,!is.na(Current_PTSD_Dx),select=c(FID,IID,Case))
 
 
 
  table(is.na(dm1_exp$Current_PTSD_Dx))
  
  
  write.table(dm1_exp,file=paste("pheno/case_control_starrs/p2_",tdata,"_nss2","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/case_control_starrs/p2_",tdata,"_",ancgroup,"_nss2","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
