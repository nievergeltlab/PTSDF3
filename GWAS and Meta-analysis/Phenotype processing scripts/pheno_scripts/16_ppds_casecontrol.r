...
which.max2 <- function(x)
{
 ...
 for (ancgroup in c("eur","lat","aam"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/case_control_starrs/p2_",tdata,"_ppds","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/case_control_starrs/p2_",tdata,"_",ancgroup,"_ppds","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
