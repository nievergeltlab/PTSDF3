...
#Dont use lifetimeDX for STARRS, its not great
 ...
 for (ancgroup in c("eur","lat","aam"))
 {
  ...
  write.table(dm1_exp,file=paste("pheno/case_control_starrs/p2_",tdata,"_nss1","_",ancgroup,".pheno",sep=""),quote=F,row.names=F)
    
  dm_cov <- dm1x  

  covexp4 <- subset(dm_cov,select=c("FID","IID","C1","C2","C3","C4","C5")) #mrs doesnt need sex!
  
  write.table(covexp4,file=paste("pheno/case_control_starrs/p2_",tdata,"_",ancgroup,"_nss1","_pcs.cov",sep=""),quote=F,row.names=F)
 }  
