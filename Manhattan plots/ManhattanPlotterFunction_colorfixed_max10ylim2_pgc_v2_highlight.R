...
ManhattanPlot_AJS_cut <- function(chr, pos, pvalue, snp, genomesig = 0, genomesug = 0,photoname = "Manhattan Plot", outname = "ManhattanPlot_AJS", replot=0, colors = c("navyblue","gray69"), sigcolor = "darkred", sugcolor = "indianred", ncex = .5, plotsymbol = 16, sugcex = 1, sigcex = 1.5, sigsnpcolor = "red", nonautosome = c(23,24,25,26),xlabel = "Chromosomal Positions",ylabel = "-log10(p-value)", pdf = "TRUE",topsnplabels = "FALSE", pvalue_miss = "NA",highlight_p=5e-800,highlightboundary=50000,pchF=16) {
 ...
 #identify proximal markers, color those too
        ...
        sigmarkers1 <- which(pvalue <= as.numeric(highlight_p)) #Adam
        most_sig <- which.min(pvalue)
        ...
        { 
         c1 <- pos[i] >= pos[sigmarkers1] - boundary_bp
         c2 <- pos[i] <= pos[sigmarkers1] + boundary_bp
         c3 <- chr[i] == chr[sigmarkers1]
         summed <- c1 + c2 + c3
         if(max(summed) == 3)
         {
          posfit[i] <- TRUE
        ...
        sigmarkers2 <- which(posfit == TRUE) #can just use the same variable, since we test each marker, they will be included in this DF.


        marker_colors <- chrominfo[,6][match(chr,chrominfo[,1])] 
        marker_colors[sigmarkers2] <-  sigsnpcolor
        marker_colors[sigmarkers1] <-  "black"

        marker_pch <- rep(plotsymbol,length(pos))
        marker_pch[sigmarkers1] <- 21
  # marker_pch[length(sigmarkers)] <- 18 #make hte last one a square -- this may not be the most significant one...
   
	#plot the points and define size of axes
 
 if(replot==0)
 {
	 plot(pos + chrominfo[,3][match(chr,chrominfo[,1])],-log10(pvalue), bg=sigsnpcolor, col = marker_colors,cex = ptcex, pch = marker_pch, ann = F, xaxt = "n", xlim = c(0,1.015*xmax), ylim = c(2, max(21)),cex.axis=1.45, cex.lab=1.5)
  } else {
   points(pos + chrominfo[,3][match(chr,chrominfo[,1])],-log10(pvalue), bg=sigsnpcolor, col = marker_colors,cex = ptcex, pch = marker_pch, ann = F, xaxt = "n", xlim = c(0,1.015*xmax), ylim = c(2, max(21)),cex.axis=1.45, cex.lab=1.5)
   
 }
 
       #adam: Replot the regions around significant markers so they show up better
