...
#Each file gets encoded with a linking column
 ...
 IFS=$'\n'
for study in $(cat f3_studies_forest.txt | tail -n+2 | grep mvp1)
do
  echo $study
...
IFS=$'\n'
for fp in $(cat f3_hits_eur.txt | tail -n+46 | head -n 32   | grep rs677 )
do
  ...
  echo $forestsnp
  #Loop over studies
  for study in $(cat f3_studies_forest.txt | grep mvp) # | tail -n+2)
  do
 ...
 # sbatch  --time=2:05:00 --error errandout/snpformatter.e --output errandout/snpformatter.o --export=ALL 06_forest_plot.sh -d WorkingDir
  ...
  IFS=$'\n'
 for fp in  $(cat f3_hits_eur.txt | tail -n+48 | head -n 30 | grep rs67736073   )  
 do
   forestsnp=$(echo $fp | awk ' {print $2}') 
...
#Rename forest plots by locus instead of rsid..

IFS=$'\n'
for locusX in $(cat locus_numbers.txt | tail -n+47 | head -n 30  )
do
 locus=$(echo $locusX | awk '{print $1}')
 rsid=$(echo $locusX | awk '{print $2}')
 
 mv results_filtered/forest_plots/"$rsid".pdf results_filtered/forest_plots/pdf/"$locus"_"$rsid".pdf 
 done
