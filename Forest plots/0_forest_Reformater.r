args <- commandArgs(trailingOnly = TRUE)
 ...
 snp <- args[4] 
    ...
    codingfile="results_filtered/forest_plots/mvp1_mvp1_eur.coding"
    snpresults="results_filtered/forest_plots/mvp1_mvp1_eur.rs67736073"
      outname="results_filtered/forest_plotsmvp1_mvp1_eur.rs67736073.formatted"
    snp="rs67736073"

library(data.table)

codingdata <- fread(codingfile,stringsAsFactors=F,header=T,nrow=2,data.table=F)
snpdata <- fread(snpresults,stringsAsFactors=F,header=T,nrow=2,data.table=F)
