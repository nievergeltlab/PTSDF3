args <- commandArgs(trailingOnly = TRUE)
 ...
 ancestry <- args[4]
    ...
    snpresults="results_filtered/forest_plots/rs67736073.allstudies"
   descriptor="f3_studies_forest_descriptors.txt"
  outfile="results_filtered/forest_plots/rs67736073"
   ancestry="eur"
...
#Load data (should be in described format of file name followed by 
 ...
 if(!is.na(any(d1$A1==TRUE)))
 {
  if(any(d1$A1==TRUE))
  {
  d1[which(d1$A1==TRUE),]$A1 <-"T"
  }
 }
  if(!is.na(any(d1$A2==TRUE)))
 {
  if(any(d1$A2==TRUE))
  {
  d1[which(d1$A2==TRUE),]$A2 <-"T"
  }
 }
...
mvpstart=1
ehrstart=mvpstart+4
ehrlimit=(results_ehr$k-1)+ehrstart
results25start=ehrlimit+4
results25limit=results_25$k+results25start-1

plotstartpoint=results25limit+4 # #The limits of the graph include extra spaces at the top of the plot, plus spaces for the headings, see forest.rma in https://cran.r-project.org/web/packages/metafor/metafor.pdf

rowpoints <- c(mvpstart,ehrstart:ehrlimit,results25start:results25limit)

forest(results,rows=sort(rowpoints,decreasing=TRUE), header="Study" ,cex=0.75,ylim=c(-1,plotstartpoint),xlim=c(-6, 6),col="blue",border="blue",mlab="Freeze 3 Meta")


#notice how I cut out an empty space for the first meta... now for the second...
#Add text for subgroups as well

#text(-6.2, c(plotstartpoint-3,(plotstartpoint-4-results_25$k-ngroups),(plotstartpoint-4-results_25$k-results_ehr$k-7)), pos=4, c("Freeze 2.5",
 #                              "EHR",
 #                              "MVP"))
                               
text(-6.2, c(mvpstart+1,ehrlimit+1,results25limit+1), pos=4, c( "MVP", "EHR" ,"Freeze 2.5"))
                          
#text(-6.2, c(ehrlimit+1,results25limit+1), pos=4, c( "EHR" ,"Freeze 2.5"))
                               
addpoly(results_25, mlab="F2.5 Meta", row= results25start-1,col="blue",border="blue",cex=0.8 )
addpoly(results_ehr,  mlab="EHR Meta",  row=ehrstart-1,col="blue",border="blue" ,cex=0.8)
#addpoly(results_mvp , mlab="MVP", row= mvpstart-)
dev.off()
